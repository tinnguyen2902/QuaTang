...
## Context
- V1: schema creation, V2: seed admin + 5 products (dien-tu, thoi-trang, sach categories)
- Cần thêm bảng `product_regions` và seed sản phẩm đủ 3 region

## Implementation Steps

### 1. Tạo file: `src/main/resources/db/migration/V3__add_product_regions.sql`
...
('Đặc Sản Địa Phương', 'dac-san-dia-phuong', 'Quà tặng đặc sản vùng miền Việt Nam', NOW()),
('Quà Tặng Quốc Tế',  'qua-tang-quoc-te',   'Sản phẩm nhập khẩu từ nước ngoài',   NOW());
...
- [ ] Tạo file V3__add_product_regions.sql với nội dung trên
- [ ] Verify Flyway chạy thành công (check log khi bootRun)
- [ ] Query DB kiểm tra data: `SELECT * FROM product_regions LIMIT 10`

## Success Criteria

- `product_regions` table tồn tại trong DB
- Mỗi region có ít nhất 6 sản phẩm
- Flyway checksum valid, không có migration conflict
